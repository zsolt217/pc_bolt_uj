...
namespace Szt2_projekt
{
    ...
    public class KompatibilitasVizsgalo
    {
        ...
        public bool Ujrabetoltes { get; set; }
        ...
        void VM_TermekValtozott(object source, KompatibilitasEventArgs e)
        {
            ...
            if (!Ujrabetoltes)
            {
                ...
                if (e.Valtozott.Equals("SelectedAlaplap"))
                {
                    ...
                    if (!VM.SelectedAlaplap.TIPUSSZAM.Contains("*")) //ha nem nincs elem kiv. 
                    {
                        List<MEMORIA> memoriak =
                            DB.MEMORIA.Where(x => x.MEMORIATIPUS.Equals(VM.SelectedAlaplap.MEMORIATIPUS)).ToList();
                        ...
                        List<CPU> cpuk = DB.CPU.Where(x => ((x.CPUFOGLALAT.Equals(VM.SelectedAlaplap.CPUFOGLALAT)) &&
                                                            (VM.SelectedTap.TIPUSSZAM.Contains("*")
                                                                ? true
                                                                : x.FOGYASZTAS < VM.SelectedTap.TELJESITMENY))).ToList();
                        //abban az esetben ha van táp kiválasztva akk csak kisebb fogyasztású tápot enged kiválasztani mint a táp teljesítménye
                        ...
                        List<HAZ> hazak =
                            DB.HAZ.Where(x => x.MERETSZABVANY.Equals(VM.SelectedAlaplap.MERETSZABVANY)).ToList();
                ...
                else if (e.Valtozott.Equals("SelectedCpu"))
                {
                    if (!VM.SelectedCpu.TIPUSSZAM.Contains("*"))
                    {
                        ...
                        List<ALAPLAP> alaplapok =
                            DB.ALAPLAP.Where(
                                x =>
                                    (VM.SelectedHaz.TIPUSSZAM.Contains("*")
                                        ? true
                                        : x.MERETSZABVANY.Equals(VM.SelectedHaz.MERETSZABVANY)) &&
                                    (VM.SelectedMemoria.TIPUSSZAM.Contains("*")
                                        ? true
                                        : x.MEMORIATIPUS.Contains(VM.SelectedMemoria.MEMORIATIPUS)) &&
                                    x.CPUFOGLALAT.Equals(VM.SelectedCpu.CPUFOGLALAT)).ToList();
                        //ha van memória vagy táp akk ennek megfelelően szűri az alaplapokat
                        ...
                        List<TAP> tapok = DB.TAP.Where(x => (x.TELJESITMENY > (VM.SelectedCpu.FOGYASZTAS)) &&
                                                            (VM.SelectedGpu.TIPUSSZAM.Contains("*")
                                                                ? true
                                                                : ((VM.SelectedCpu.FOGYASZTAS +
                                                                    VM.SelectedGpu.FOGYASZTAS) <= x.TELJESITMENY)))
                            .ToList();
                        //ha van gpu kiválasztva akk a cpuval együtt a fogyasztásnak kisebb egyenlőnek kelllennie a táp teljestíményével
                        ...
                        List<ALAPLAP> alaplapok =
                            DB.ALAPLAP.Where(
                                x =>
                                    ...
                                    (VM.SelectedMemoria.TIPUSSZAM.Contains("*")
                                        ? true
                                        : x.MEMORIATIPUS.Contains(VM.SelectedMemoria.MEMORIATIPUS))).ToList();
                        //ha van memória vagy táp akk ennek megfelelően szűri az alaplapokat
                        alaplapok.Add(new ALAPLAP { TIPUSSZAM = "*nincs elem kivalasztva" });
                        if (!VM.SelectedAlaplap.TIPUSSZAM.Contains("*"))
                        {
                        ...
                        List<TAP> tapok =
                            DB.TAP.Where(
                                x =>
                                    (VM.SelectedGpu.TIPUSSZAM.Contains("*")
                                        ? true
                                        : ((VM.SelectedGpu.FOGYASZTAS) <= x.TELJESITMENY))).ToList();
                        //ha van gpu kiválasztva akk a cpuval együtt a fogyasztásnak kisebb egyenlőnek kelllennie a táp teljestíményével
                        if (!VM.SelectedTap.TIPUSSZAM.Contains("*"))
                        {
                            TAP selected = VM.SelectedTap;
                    ...
                    if (!VM.SelectedGpu.TIPUSSZAM.Contains("*"))
                    {
                        List<TAP> tapok =
                            DB.TAP.Where(
                                x =>
                                    x.TELJESITMENY >=
                                    ((VM.SelectedCpu.TIPUSSZAM.Contains("*") ? 0 : VM.SelectedCpu.FOGYASZTAS) +
                                     VM.SelectedGpu.FOGYASZTAS)).ToList();
                        tapok.Add(new TAP { TIPUSSZAM = "*nincs elem kivalasztva" });
                        if (!VM.SelectedTap.TIPUSSZAM.Contains("*"))
                        {
                            TAP selected = VM.SelectedTap;
                    ...
                    else
                    {
                        List<TAP> tapok =
                            DB.TAP.Where(
                                x =>
                                    x.TELJESITMENY >=
                                    ((VM.SelectedCpu.TIPUSSZAM.Contains("*") ? 0 : VM.SelectedCpu.FOGYASZTAS))).ToList();
                        tapok.Add(new TAP { TIPUSSZAM = "*nincs elem kivalasztva" });
                        if (!VM.SelectedTap.TIPUSSZAM.Contains("*"))
                        {
                            TAP selected = VM.SelectedTap;
                ...
                else if (e.Valtozott.Equals("SelectedMemoria"))
                {
                    ...
                    {
                        List<ALAPLAP> alaplapok =
                            DB.ALAPLAP.Where(x => x.MEMORIATIPUS.Equals(VM.SelectedMemoria.MEMORIATIPUS) &&
                                                  (VM.SelectedHaz.TIPUSSZAM.Contains("*")
                                                      ? true
                                                      : x.MERETSZABVANY.Equals(VM.SelectedHaz.MERETSZABVANY)) &&
                                                  (VM.SelectedCpu.TIPUSSZAM.Contains("*")
                                                      ? true
                                                      : x.CPUFOGLALAT.Equals(VM.SelectedCpu.CPUFOGLALAT))).ToList();
                        alaplapok.Add(new ALAPLAP { TIPUSSZAM = "*nincs elem kivalasztva" });
                        if (!VM.SelectedAlaplap.TIPUSSZAM.Contains("*"))
                        ...
                        List<ALAPLAP> alaplapok =
                            DB.ALAPLAP.Where(
                                x =>
                                    (VM.SelectedMemoria.TIPUSSZAM.Contains("*")
                                        ? true
                                        : x.MEMORIATIPUS.Equals(VM.SelectedMemoria.MEMORIATIPUS)) &&
                                    (x.MERETSZABVANY.Contains(VM.SelectedHaz.MERETSZABVANY)) &&
                                    (VM.SelectedCpu.TIPUSSZAM.Contains("*")
                                        ? true
                                        : x.CPUFOGLALAT.Equals(VM.SelectedCpu.CPUFOGLALAT))).ToList();
                ...
                else if (e.Valtozott.Equals("SelectedTap"))
                {
                    ...
                    if (!VM.SelectedTap.TIPUSSZAM.Contains("*"))
                    {
                        List<CPU> cpuk =
                            DB.CPU.Where(
                                x =>
                                    x.FOGYASZTAS < VM.SelectedTap.TELJESITMENY &&
                                    (VM.SelectedAlaplap.TIPUSSZAM.Contains("*")
                                        ? true
                                        : (x.CPUFOGLALAT.Equals(VM.SelectedAlaplap.CPUFOGLALAT)))).ToList();
                        cpuk.Add(new CPU { TIPUSSZAM = "*nincs elem kivalasztva" });
                        if (!VM.SelectedCpu.TIPUSSZAM.Contains("*"))
                        {
                            CPU selected = VM.SelectedCpu;
                    ...
                    {
                        List<CPU> cpuk =
                            DB.CPU.Where(x => x.CPUFOGLALAT.Equals(VM.SelectedAlaplap.CPUFOGLALAT)).ToList();
                        //abban az esetben ha van táp kiválasztva akk csak kisebb fogyasztású tápot enged kiválasztani mint a táp teljesítménye
                        cpuk.Add(new CPU { TIPUSSZAM = "*nincs elem kivalasztva" });
                        if (!VM.SelectedCpu.TIPUSSZAM.Contains("*"))
                        {
